...
if (!function_exists('generate_color_console')) {
...
if (!function_exists('get_binding_names_from_insert')) {
    /**
...
if (!function_exists('get_binding_names_from_expressions')) {
    /**
...
if (env('APP_DEBUG') === true) {
    ...
    DB::listen(function ($query) use ($output) {
        ...
        if ($query->bindings) {
            ...
            if (count($names) !== count($query->bindings)) {
        ...
        if (config('app.env') === 'local') {
            Log::channel('query')->info(\Str::of(strip_tags($str))->trim());
        }
    });
}
